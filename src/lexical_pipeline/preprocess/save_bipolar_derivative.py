...
def preproc(
    ...
    **kwargs
    ...
    # Patch dataset_description.json so PyBIDS recognises it as a valid derivative
    desc_path = Path(bids_layout.root) / 'derivatives' / 'bipolar' / 'dataset_description.json'
    if desc_path.exists():
        ...
        if 'GeneratedBy' not in desc:
            desc['GeneratedBy'] = [{'Name': 'save_bipolar_derivative'}]
            changed = True
        if changed:
            with open(desc_path, 'w') as f:
                json.dump(desc, f, indent=4)
            logger.info(f'Patched {desc_path} with GeneratedBy and DatasetType')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bids_root", default="/cwork/jq81/cogan_lab_box/CoganLab/BIDS-1.0_LexicalDecRepNoDelay/BIDS", type=str)
